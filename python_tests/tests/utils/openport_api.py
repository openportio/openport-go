...
from tests.utils.openport_app_version import VERSION
from tests.utils import osinteraction

logger = get_logger("openport_api")
DEFAULT_SERVER = "https://test.openport.io"
...
def request_port(
    ...
    try:
        ...
        verify = (
            "local" not in url
            and "192.168" not in url
            and not os.environ.get("NO_SSL_VERIFY")
        )
        ...
        if e.response:
            ...
            if e.response.status_code == 500:
                try:
                    ...
                    with open(error_page_file_path, "w") as f:
                        logger.debug(
                            "error is available here: %s" % error_page_file_path
                        )
...
def request_open_port(
    ...
    logger.debug(
        "requesting port forward - remote port: %s, restart session token: %s"
    ...
    if request_server_port != -1 and request_server_port != response.remote_port:
        logger.error(
            "Did not get requested server port (%s), but got %s"
