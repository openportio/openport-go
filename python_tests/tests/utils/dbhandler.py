...
class DBHandler(object):
    def __init__(self, db_location=None, echo_queries=False):
    ...
    def add_share(self, share):
        ...
        if share.id > 0:
            openport_session = (
                session.query(OpenportSession).filter_by(id=share.id).first()
            )

        if openport_session is None:
            openport_session = (
                session.query(OpenportSession)
                .filter_by(local_port=share.local_port)
                .first()
            )
        ...
        for other_session in session.query(OpenportSession).filter_by(
    ...
    def convert_session_from_db(self, openport_session: OpenportSession) -> Session:
        ...
        b = (
            ...
            if type(share.restart_command) == bytes
            else share.restart_command.encode("utf-8")
        )
        try:
    ...
    def get_active_shares(self) -> List[OpenportSession]:
        ...
        l = list(
            self.convert_session_from_db(openport_session)
            for openport_session in openport_sessions
        )
        self.Session.remove()
        return l
    ...
    def get_shares_to_restart(self):
        ...
        session = self._get_session()
        openport_sessions = session.query(OpenportSession).filter(
            OpenportSession.restart_command.isnot("")
        )
    ...
    def get_share_by_local_port(self, local_port, filter_active=True):
        ...
        openport_session = (
            session.query(OpenportSession)
            .filter_by(**kwargs)
            .order_by(OpenportSession.id.desc())
            .first()
        )
