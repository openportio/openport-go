...
class OsInteraction(object):
    ...
    def get_full_restart_command(self, share):
        if not share.restart_command:
            self.logger.debug(
                "no restart command for share with local port %s" % share.local_port
            )
            return
        # Legacy...
    ...
    def run_command_and_print_output_continuously(
        self, command_array, prefix="", cwd=None, shell=False
    ):
    ...
    def get_openport_exec(self):
        if self.is_compiled():
            ...
            path = self.quote_path(
                os.path.join(os.path.dirname(sys.argv[0]), "openportw.exe")
            )
            if not os.path.exists(path):
                path = self.quote_path(
                    os.path.join(os.path.dirname(sys.argv[0]), "openport.exe")
                )
            if not os.path.exists(path):
                path = self.quote_path(
                    os.path.join(os.path.dirname(sys.argv[0]), "openport")
                )
            if not os.path.exists(path):
    ...
    def pid_is_openport_process(self, pid):
        ...
        for i in process:
            return (
                "openport" in i.name() or "python" in i.name()
            )  # Is not 100% but best effort. (for mac)
        return False
    ...
    def run_function_with_lock(
        self, function, lock_file, timeout=30, args=[], kwargs={}
    ):
    ...
    def resource_path(*relative_paths):
        ...
        if hasattr(sys, "_MEIPASS"):
            ...
            filename = os.path.join(
                os.path.dirname(__file__), "../../openport", *relative_paths
            )
...
class LinuxOsInteraction(OsInteraction):
    ...
    def is_compiled(self):
        return (
            sys.argv[0] != "python"
            and sys.argv[0][-3:] != ".py"
            and not "nosetests" in sys.argv[0]
        )
